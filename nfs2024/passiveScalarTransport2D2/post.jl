...
plotly(); quiver(X,Y,quiver=(U*1e-7,V*1e-7))

# ╔═╡ f0ab5d2a-e418-4442-a64f-d8dd31968afe
step = @bind step Slider(0:saveNT:5000, show_value=true)

# ╔═╡ 170b5645-148e-4100-a321-87f32fc56e15
dummy
...
let
	...
	anim = @animate for i in 0:saveNT:nTMax
		path = datPath("Phi", i)
		readDat!(dummy2, path)
		heatmap(dummy2[Ifi:Ila, Jfi:Jla], size=(700,700), clims=(-1.2,1.2), c=:Greys, cbar=false, ticks = false)
	end
	gif(anim, "TaylowGreenVortex.gif")
end
  ╠═╡ =#
...
begin
	...
	println(path)
	readDat!(dummy, path)
	#surface(dummy[Ifi:Ila, Jfi:Jla], size=(700,700), clims=(-1.2,1.2), c=:Greys		
#, cbar=false, ticks = false)
		heatmap(dummy, size=(700,700), clims=(-1.2,1.2), c=:Greys		
, cbar=true, ticks = false)
