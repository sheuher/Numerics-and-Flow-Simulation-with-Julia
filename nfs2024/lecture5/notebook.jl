...
let
	x = 1; CFL = 0.25; U = 1.; D = 1e-5

	nItMax = 10000; epsMax = 1e-6
	...
	divPred = similar(rhouP); p = similar(rhouP)
	rhou = similar(rhouP)
	...
	rhouP[Ifi:Ila] .= LinRange(DeltaX/2,x - DeltaX/2,Ima) .|> t->cos(2π*t)
	rhouP[Ifim] = rhouP[Ila]; rhouP[Ilap] = rhouP[Ifi]
	
	divMom1D!(divPred, rhouP, DeltaX)
	poissonSolver1D!(p, divPred, DeltaT, DeltaX, nItMax, epsMax)
